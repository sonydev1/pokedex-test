...
function PokeCard({ id, name, img, habi, color, type }) {
  if (
    ...
    return (
      <>
        <div className="flex flex-col items-center sm:flex sm:flex-row sm:gap-5">
          ...
          <section className=" w-64 h-80  text-center ">
            ...
            <div className="relative px-1 w-30 mt-4 ">
              <img
                src={pokebal}
                alt={name}
                className="absolute top-0 w-96 animate-[spin_10s_linear_infinite]  "
              />
              <img src={img} alt={name} className=" w-60 drop-shadow-sm" />
            </div>
          </section>
          <section className=" mt-8 w-80    bg-white rounded-2xl bg-opacity-50">
